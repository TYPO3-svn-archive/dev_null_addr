 ...
 */
 class tx_devnulladdr_pi1_mapjs extends tslib_pibase {
	public $prefixId      = 'tx_devnulladdr';		// Same as class name
	public $scriptRelPath = 'pi1/class.tx_devnulladdr_pi1.map_js.php';	// Path to this script relative to the extension dir.
	...
	public function main($content, array $conf) {
		...
		// $this->uidMap = $this->piVars['map'];
		
		// load propper tt_content record
		// $this->cObj->start($this->pi_getRecord('tt_content', $this->uidMap), 'tt_content');
		...
		// $content = t3lib_utility_Debug::viewArray($this->conf);
		// $content .= t3lib_utility_Debug::viewArray($this->cObj->data);
		// return t3lib_utility_Debug::viewArray($this->cObj->data);
		...
		// Prepare select statement for tx_devnulladdr_kml_file list
		$dbKmlFields = 'tx_devnulladdr_map_layers_mm.*, tx_devnulladdr_kml_file.*';
		
		$dbKmlTables = 
			'tx_devnulladdr_map_layers_mm inner join tx_devnulladdr_kml_file
				on tx_devnulladdr_map_layers_mm.uid_foreign = tx_devnulladdr_kml_file.uid';
		
		$dbKmlWhere = implode(' AND ', array(
				'tx_devnulladdr_map_layers_mm.matchfield = :ident',
				'tx_devnulladdr_map_layers_mm.tablenames = :table',
				'tx_devnulladdr_map_layers_mm.uid_local = :uid',
		));
		...
		$sqlKml = $GLOBALS['TYPO3_DB']->prepare_SELECTquery($dbKmlFields, $dbKmlTables, $dbKmlWhere, '', '');
		
		$sqlKml->execute(array(
			':uid' => $this->uidMap,
			':ident' => 'tx_devnulladdr_pi1',
			':table' => 'tx_devnulladdr_kml_file'
		));

		while($rowKml = $sqlKml->fetch()){
			if($rowKml['type'] == 1)
				$link = $rowKml['kml_url'];
			else
				$link = $this->cObj->typoLink_URL(array('parameter' => $rowKml['kml_file']));

			$markers = array(
				'###MAP_UID###' => $this->uidMap,
				'###KML_UID###' => $rowKml['uid'],
				'###KML_URL###' => $link,
				'###KML_SUPPRESS###' => $rowKml['kml_suppress'] ? 'true' : 'false',
				'###KML_PRESERVE###' => $rowKml['kml_preserve'] ? 'true' : 'false',
			);
			$script .= $this->cObj->substituteMarkerArray($subParts['kml'], $markers);
		}
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/dev_null_addr/pi1/class.tx_devnulladdr_pi1.map_js.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/dev_null_addr/pi1/class.tx_devnulladdr_pi1.map_js.php']);
}
